...
<body>
    ...
    if (isset($_POST['btnsearch'])) {
        ...
        $innerSql = "SELECT * FROM tbproductinfo WHERE itemid LIKE '%" . $id . "%' OR itemname LIKE '%" . $id . "%'";
        ...
        if ($innerResult->num_rows > 0) {
            ...
            while ($row = $innerResult->fetch_assoc()) { 
                ...
                <div class="col m-2"
                    ...
                    <div class="card " style="width:18rem;border-radius:25px;height:auto;box-shadow:5px 8px 11px rgba(179,0,0,0.6);">
                        ...
                        <div class="card-body" style = "background-color:transparent">
                            ...
                            <div class = "row row-col-3">
                                <a href="updateitem.php?id=<?=$id ?>&name=<?=$name?>&image=<?=$base64IMG?>&size=<?=$size?>&price=<?=$price?>" class="btn btn-success col">Edit</a>
                                <a href="deleteitem.php?id=<?=$id ?>&name=<?=$name?>&image=<?=$base64IMG?>&size=<?=$size?>&price=<?=$price?>" class="btn btn-danger col">Delete</a>
                                <?php 
                                    if(isset($_GET['message'])){
                                       $message = $_GET['message'];
                                       echo' <a class="btn btn-outline-successs col">{$message}</a>';
                                    }
                                ?>
                            </div>
                        </div>
